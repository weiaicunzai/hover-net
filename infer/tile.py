...
def _post_process_patches(
    ...
    print('----------------------------')
    print(patch_data.shape, type(patch_data))
    #import sys; sys.exit()
    ...
    ch = 1 if len(patch_shape) ==patch_shape 2 else patch_shape[-1]
...
class InferManager(base.InferManager):
    ...
    def process_file_list(self, run_args):
        ...
        def detach_items_of_uid(items_list, uid, nr_expected_items):
            print(len(items_list), uid, nr_expected_items)
            item_counter = 0
            detached_items_list = []
            remained_items_list = []
            while True:
        ...
        #################################################
            while len(file_path_list) > 0:
                ...
                for batch_idx, batch_data in enumerate(dataloader):
                    ...
                    #print(11, sample_output_list.shape) # (8, 80, 80, 3)
                    ...
                    #print('jfjfjjfjfj', len(sample_output_list), sample_output_list)
                    sample_output_list = list(zip(sample_info_list, sample_output_list))
                    # print(sample_output_list[0][1].shape) # (1, 80, 80, 3)
                ...
                for file_idx, file_path in enumerate(use_path_list):
                    ...
                    #print(len(accumulated_patch_output), len(accumulated_patch_output[0]), len(accumulated_patch_output[0][0]))
                    # 108 2 1
                    # print(accumulated_patch_output[0][1].shape) (1, 80, 80, 3)
                    print(image_info[1], file_idx)
                    file_ouput_data, accumulated_patch_output = detach_items_of_uid(
                        accumulated_patch_output, file_idx, image_info[1]
                    )
                    print(len(file_ouput_data))
                    #print(accumulated_patch_output[0][1].shape) # (1, 80, 80, 3)
                    #print(accumulated_patch_output[3][3][1].shape)
